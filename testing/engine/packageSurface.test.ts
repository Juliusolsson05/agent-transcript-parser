...
describe('package composition surface', () => {
  it('exports neutral composition and independent provider adapters', () => {
    ...
    expect(engine.piConversationDecoder.provider).toBe('pi')
    expect(engine.piArchiveProjector.provider).toBe('pi')
    expect(engine.piNativeResumeProjector.profile.provider).toBe('pi')
